...
public class SetTime extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
    	...
    	try{
			...
			sSql = "";
			sSql = sSql.concat("select tbl_account.name as name, cast(tbl_attendance.cometime as char) as cometime, cast(tbl_attendance.returntime as char) as returntime "																	);
			...
			sSql = sSql.concat("; "																						);
			// SQL実行
			tbl_printmenu = MyQuery.selectTbl_PrintMenu(sSql);
			if(tbl_printmenu.iGetFlag != 1){
				// UID取得に失敗したときの処理
				throw new SQLException();
			}
